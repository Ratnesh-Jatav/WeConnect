...
const EditFamilyMemberModal = ({ member, onClose, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await familyMemberService.update(member._id, formData);
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 p-5" onClick={onClose}>
      <div className="relative max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="mb-5 text-2xl font-semibold text-slate-800">Edit Family Member</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="mt-5 flex justify-end gap-2.5">
            ...
            <button type="submit" className="rounded-lg bg-indigo-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-indigo-600 disabled:opacity-60" disabled={loading}>{loading ? 'Updating...' : 'Save Changes'}</button>
          </div>
        </form>
      </div>
    </div>
